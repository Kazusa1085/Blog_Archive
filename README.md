...
music/
  config.json  音乐播放器配置（侧边栏卡片）。这个文件不存在时播放器默认禁用；
               存在时还可以用 "enable": false 临时关掉而不用删文件。
```
...
- `music/config.json`：
  - `enable`（可选，默认 `true`，文件存在即视为启用，除非显式设为 `false`）
  - `mode`：`"meting"` 或 `"local"`
  - `mode: "meting"` 时：`meting_api`、`server`、`type`、`id`（歌单/专辑/歌手 id，取决于 `type`）——通过 [Meting API](https://github.com/metowolf/Meting) 从网易云/QQ音乐/酷狗等平台拉取歌单，不用自己传音频文件
  - `mode: "local"` 时：`localPlaylist`，一个歌曲数组，每首歌 `{ id, title, artist, cover, url, duration }`，`url`/`cover` 建议用完整外部链接（这个仓库不适合放音频文件本身）
